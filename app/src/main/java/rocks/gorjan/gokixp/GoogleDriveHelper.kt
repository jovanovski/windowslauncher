...
/**
 * Keeps one copy of the [PrefsBackup] JSON in the user's Google Drive.
 *
 * This lived in the Registry Editor's package while that program was the only way to reach
 * it; Backup & Restore in Display Properties is now, and the sync runs whether or not any
 * window is open, so it sits beside [PrefsBackup] instead.
 */
class GoogleDriveHelper(private val context: Context) {
    ...
    companion object {
        ...
        /**
         * Named for the Registry Editor it was written from, and it has to stay that way:
         * this is the file every existing backup is already sitting in on Drive.
         */
